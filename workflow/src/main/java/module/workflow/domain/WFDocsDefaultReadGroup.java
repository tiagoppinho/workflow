 ...
 *         delegates the access control to a mixture between
 *         {@link WorkflowProcess#isFileSupportAvailable()} and
...
public class WFDocsDefaultReadGroup extends WFDocsDefaultReadGroup_Base {
    ...
    /**
     * Only used by _Base, not to use directly, use
     * {@link WFDocsDefaultReadGroup#WFDocsDefaultReadGroup(WorkflowProcess)}
     * instead
     */
    @Deprecated
	...
	WFDocsDefaultReadGroup readGroup = (process.getDocumentsRepository() == null || !(process.getDocumentsRepository()
		.getReadGroup() instanceof WFDocsDefaultReadGroup)) ? null
		: (WFDocsDefaultReadGroup) process
