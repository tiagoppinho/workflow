...
public class ProcessDirNode extends ProcessDirNode_Base {
 ...
 /*   static {
	...
	if (nodeFound != null)
	{
	    existingProcessDocument = getProcessFile((FileNode) nodeFound);
	    if (!(existingProcessDocument.getClass().equals(processDocClass)))
		throw new WorkflowDomainException("no.files.same.name.allowed.rename.pls", displayName);
	}
	
	FileNode fileNode = getDirNode()
		.createFile(content, displayName, fileName, content.length, new ContextPath(getDirNode()));
