...
public class WFDocsDefaultWriteGroup extends WFDocsDefaultWriteGroup_Base {
    ...
    /**
     * For use with _Base only. Use {@link WFDocsDefaultWriteGroup#WFDocsDefaultWriteGroup(WorkflowProcess)} instead
     */
    @Deprecated
    ...
    public static module.workflow.domain.AbstractWFDocsGroup getOrCreateInstance(WorkflowProcess process) {
        WFDocsDefaultWriteGroup writeGroup =
                (process.getDocumentsRepository() == null || !(process.getDocumentsRepository().getReadGroup() instanceof WFDocsDefaultWriteGroup)) ? null : (WFDocsDefaultWriteGroup) process
