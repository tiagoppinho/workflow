...
public class AssignDirNodeRepositoryToProcessAndMigrate extends ReadCustomTask {
	...
	public void transactionalRun() {
	    ...
	    //touching the groups, giggly (to make sure they are on the readset of this thread, yes, this is needed, yep I only found out a week ago..)
	    for (PersistentGroup group : persistentGroupsToDelete) {
		AbstractDomainObject.fromExternalId(group.getExternalId());
	    }
	    for (PersistentGroup group : persistentGroupsToDelete) {
		try {
