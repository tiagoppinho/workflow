 ...
 * 
 *         Used to manage all of the things that are dependent of a
 *         {@link WorkflowMetaTypeVersion} i.e. states, fields, description
 * 
 */
...
public class ManageMetaTypeVersionComponent extends CustomComponent implements EmbeddedComponentContainer {
			...
			for (Button button : getVersionActionButtonsList())
 {
	...
	Label stateLabel = metaTypeVersion.getPublished() ? new Label("- " + getMessage("label.published") + " -") : new Label(
	...
	private void initVersionPublished(WorkflowMetaTypeVersion metaTypeVersion) {
	    ...
	    String dateOfPublication = metaTypeVersion.getDatePublication() == null ? getMessage("label.publicationDateUnavailable") : metaTypeVersion.getDatePublication().toString();
	    ...
	    String username = metaTypeVersion.getPublisherOfVersion() == null ? null : metaTypeVersion.getPublisherOfVersion()
		    .getPresentationName();
	    ...
	    Label motiveValue = new Label("<em>" + StringEscapeUtils.escapeHtml(metaTypeVersion.getPublicationMotive()) + "</em>");
