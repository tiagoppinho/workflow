...
//$Id: Diff.java 1257 2003-12-11 13:36:55Z leo $
...
class fileInfo {
	...
	file = new DataInputStream(new StringBufferInputStream(filename));
 ...
 * is taken from Communications of the ACM, Apr78 (21, 4, 264-),
 * "A Technique for Isolating Differences Between Files." Ignoring I/O, and
 * ignoring the symbol table, it should take O(N) time. This implementation
 * takes fixed space, plus O(U) space for the symbol table (where U is the
 * number of unique lines). Methods exist to change the fixed space to O(N)
 * space. Note that this is not the only interesting file-difference algorithm.
 * In general, different algorithms draw different conclusions about the changes
 * that have been made to the oldfile. This algorithm is sometimes "more right",
 * particularly since it does not consider a block move to be an insertion and a
 * (separate) deletion. However, on some files it will be "less right". This is
 * a consequence of the fact that files may contain many identical lines
...
public class Diff {
	...
	Diff d = new Diff();
	System.out.println(d.doDiff(argstrings[0], argstrings[1]));
	return;
    }
    ...
    public static String doDiffX(String oneString, String otherString) {
	Diff diff = new Diff();
	return diff.doDiff(oneString, otherString);
    }

    /** Do one file comparison. Called with both filenames. */
    public String doDiff(String oldFile, String newFile) {
	...
	if (anyprinted == true)
	    builder.append(">>>> End of differences.");
	else
	    builder.append(">>>> Files are identical.");
     ...
     */
    void newconsume(StringBuilder builder) {
	for (;;) {
	    if (printnewline > newinfo.maxLine)
		break; /* end of file */
	    if (newinfo.other[printnewline] < 0)
     ...
     */
    void oldconsume(StringBuilder builder) {
	for (;;) {
	    if (printoldline > oldinfo.maxLine)
		break; /* end of file */
     ...
     */
    void showdelete(StringBuilder builder) {
	if (printstatus != delete)
	    builder.append(">>>> DELETE AT " + printoldline);
	printstatus = delete;
	oldinfo.symbol[printoldline].showSymbol(builder);
	anyprinted = true;
	printoldline++;
    }
     ...
     */
    void showinsert(StringBuilder builder) {
	if (printstatus == change)
	    builder.append(">>>>     CHANGED TO");
	else if (printstatus != insert)
	    builder.append(">>>> INSERT BEFORE " + printoldline);
	printstatus = insert;
	newinfo.symbol[printnewline].showSymbol(builder);
	anyprinted = true;
	printnewline++;
    }
     ...
     */
    void showchange(StringBuilder builder) {
	if (printstatus != change)
	    builder.append(">>>> " + printoldline + " CHANGED FROM");
	printstatus = change;
	oldinfo.symbol[printoldline].showSymbol(builder);
	anyprinted = true;
	printoldline++;
     ...
     */
    void showsame(StringBuilder builder) {
	int count;
	printstatus = idle;
	if (newinfo.other[printnewline] != printoldline) {
	    builder.append("BUG IN LINE REFERENCING");
	    System.exit(1);
	}
     ...
     */
    void showmove(StringBuilder builder) {
	...
	if (newblock < 0)
	    ...
	    builder.append(">>>> " + newother + " THRU " + (newother + newblock - 1) + " MOVED TO BEFORE " + printoldline);
...
class node { /* the tree is made up of these nodes */
     ...
     */
    void showSymbol(StringBuilder builder) {
	builder.append(line);
    }
}
